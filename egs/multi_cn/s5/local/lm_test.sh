...
if [ $stage -le 1 ]; then
  ...
  utils/mkgraph.sh data/lang_$LM exp/tri1b $graph_dir || exit 1;

  for c in $test_sets; do
    decode_dir=exp/tri1b/decode_${c}_${LM}

    steps/decode.sh --cmd "$decode_cmd" --config conf/decode.config --nj 10 \
      $graph_dir data/$c/test $decode_dir || exit 1;
  done
fi
...
if [ $stage -le 2 ]; then
  ...
  utils/mkgraph.sh data/lang_$LM exp/tri2a $graph_dir || exit 1;
...
if [ $stage -le 3 ]; then
  ...
  utils/mkgraph.sh data/lang_$LM exp/tri3a $graph_dir || exit 1;
...
if [ $stage -le 4 ]; then
  ...
  utils/mkgraph.sh data/lang_$LM exp/tri4a $graph_dir || exit 1;
...
if [ $stage -le 5 ]; then
  ...
  utils/mkgraph.sh data/lang_$LM ${cleaned_dir} $graph_dir
  ...
  for c in $test_sets; do
    ...
    steps/decode_fmllr.sh --nj $decode_nj --num-threads $decode_num_threads \
      --cmd "$decode_cmd" \
      $graph_dir data/$c/test $decode_dir
  done
fi
