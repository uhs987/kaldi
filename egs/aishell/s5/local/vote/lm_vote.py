...
def init_vote_data_directory(decode_root, vote_dir, test_set, lm_tests):
	# create data directory
	data_dir = vote_dir + '/data'
	os.mkdir(data_dir)

	# copy transcript file of ground truth to data directory
	transcript_file = './data/' + test_set + '/text'
	if os.path.exists(transcript_file) == False:
		logging.error('transcript file does not exist, path %s' % (transcript_file))
		return []

	shutil.copyfile(transcript_file, data_dir + '/test_filt.txt')

	# copy/append hypothesis file(s) of each LM to data directory
	hypothesis_files = []
	for i in range(len(lm_tests)):
		decode_dir = decode_root + '/decode_' + test_set + '_' + lm_tests[i]
...
def main():
	...
	if action == 'vote' or action == 'lcs':
		...
		vote_dir += test_set + '_' + weight_function + '_' + direction
		...
		hypothesis_files = init_vote_data_directory(decode_root, vote_dir, test_set, lm_tests)
		...
		vote_dir_common += test_set + '_' + weight_function
		...
		ngram_dir = decode_root + '/decode_' + test_set
